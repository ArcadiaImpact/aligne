...
| `introspect` | `run_introspection(IntrospectConfig)` | OCT stage 2: self-reflection + self-interaction → `sft_data.jsonl` (train with `aligne train sft --load-checkpoint-path ...`) |
| `pairs` | `run_pairs_gen(PairsConfig)` | OCT DPO comparisons (chosen = in-character, rejected = plain) → `aligne train dpo --pairs` |
...
```python
from aligne.character import PreferenceEvalConfig, run_preference_eval
...
src/aligne/character/
  constitution.py       Constitution (v1 traits / v2 values+tradeoffs) + system_block
  constitutions/        34 characters: humor, goodness, loving, candid_advisor(+flat),
                        thoughtful_assistant, risk_averse/seeking, honest_*/kind_*/lie_* pairs, ...
  prompts.py            prompt sets (load/resolve; decoupled from constitutions)
  prompts/              humor_seeds, goodness_seeds, risk_seeds, alpaca2k, *_train, ...
  exemplars.py+/        few-shot exemplar sets for the prompted teacher
  scenarios/ (+eval_coherence.py)  value-conflict scenarios + the answer key
  drivers.py            the config dataclasses + async run_* drivers (the library surface)
  eval_preferences.py   revealed-preferences eval internals
  eval_coherence.py     coherence eval internals
  eval_predictability.py predictability eval internals
  introspection.py      OCT stage-2 generation
  gen_pairs.py          OCT DPO pair generation
  cli.py                aligne character: render|distill|introspect|pairs|eval|coherence|predictability
```
