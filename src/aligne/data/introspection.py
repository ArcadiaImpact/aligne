...
from aligne.data.constitution import Constitution, trait_string
...
async def run_introspection(cfg: IntrospectConfig) -> dict[str, list[dict]]:
    ...
    free = await generate_interactions(
        cfg.checkpoint, cfg.model, cfg.renderer, name, con,
    ...
    leading = await generate_interactions(
        cfg.checkpoint, cfg.model, cfg.renderer, name, con,
    ...
    sft = build_sft_data(name, reflections, free, leading, seed=cfg.seed)
