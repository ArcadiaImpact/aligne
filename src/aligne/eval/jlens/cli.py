"""`aligne jlens` / `python -m aligne.eval.jlens.cli` — fit J-lenses from a YAML
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        from aligne.eval.jlens.fit import fit, load_config
    except ImportError as e:  # torch/transformers/safetensors/pyyaml missing
        raise SystemExit(
            f"aligne.eval.jlens needs the jlens extra: pip install 'aligne[jlens]' ({e})"
        )
