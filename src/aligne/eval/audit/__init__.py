...
`aligne.eval.audit.tenets` is dependency-light (stdlib only). `aligne.eval.audit.run` and
`aligne.eval.audit.analyze` import `inspect_petri` / `inspect_ai` lazily, so a plain
`import aligne.eval.audit` stays cheap. LLM calls (validation, decomposition) go
through the shared async `aligne.util.client.ChatClient` and run concurrently; the
CLI adapters live in `aligne.eval.audit.cli`.
"""
