...
The only argparse/stdout in ``aligne.eval.audit`` (mirrors ``aligne.eval.jlens.cli``):

    python -m aligne.eval.audit.cli analyze ./audit_logs [--validator MODEL ...]
    python -m aligne.eval.audit.cli decompose constitution.md --out tenets.json

Both build a shared :class:`aligne.util.client.ChatClient` (default: OpenRouter via
``OPENROUTER_API_KEY``) and run the async library entry points
(:func:`aligne.eval.audit.analyze.analyze_logs`,
:func:`aligne.eval.audit.decompose.decompose`).
"""
...
def main_analyze(argv: list[str] | None = None) -> None:
    from aligne.eval.audit.analyze import analyze_logs, markdown_report

    ap = argparse.ArgumentParser(
        description="Flag + validate audit logs (see aligne.eval.audit.analyze)."
    )
...
def main_decompose(argv: list[str] | None = None) -> None:
    from aligne.eval.audit.decompose import coverage_report, decompose

    ap = argparse.ArgumentParser(
        description="Constitution -> tenets.json (see aligne.eval.audit.decompose)."
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    if cmd not in commands:
        print(f"usage: python -m aligne.eval.audit.cli {{{','.join(commands)}}} [options]",
              file=sys.stderr)
